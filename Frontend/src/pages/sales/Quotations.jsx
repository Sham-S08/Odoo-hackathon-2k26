...
import { MOCK_QUOTATIONS } from "../../utils/salesMockData";
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center rounded-lg border border-slate-200 p-0.5">
              <button
                ...
                className={`rounded-md p-1.5 ${view === "list" ? "bg-blue-600 text-white" : "text-slate-400"}`}
              >
                <List className="h-4 w-4" />
              </button>
              <button
                onClick={() => setView("grid")}
                className={`rounded-md p-1.5 ${view === "grid" ? "bg-blue-600 text-white" : "text-slate-400"}`}
              >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredQuotations.map((q) => (
            <QuotationCard
              key={q.id}
              quotation={q}
              onOpen={() => navigate(`/sales/quotations/${q.id}`)}
            />
          ))}
        </div>
      )}
